...
var Averto = class Averto {
    ...
    throw (data) {
        if (!this.visible && data.message != undefined) {
            ...
            this.box.style.boxShadow = '0px 12px 85px 0px rgba(' + RGB.r + ', ' + RGB.g + ', ' + RGB.b + ', 0.61)';
            let contrastLevel = Math.round(((RGB.r * 299) +
                (RGB.g * 587) +
                (RGB.b * 114)) / 1000);
            ...
            window.requestAnimationFrame(() => {
                if (blocking)
                    this.wrapper.classList.add('is__blocking');
...
//     opacity: 0;
//     transition: opacity .2s .15s ease-out,transform .5s cubic-bezier(0.43, 0.18, 0.78, 0.36);
//     color: #fff;
//     max-width: 470px;
//     min-width: 270px;
//     border-radius: 4px;
...
//     transition: opacity .25s ease-out,transform .5s cubic-bezier(0.075, 0.82, 0.165, 1);
// }
// .averto-gradient {
//     padding: 22px 23px;
//     box-shadow:0px 16px 31px -2px rgba(0, 0, 0, 0.17);
...
//     font-weight:bold
// }
// .averto-title, .avertoBox-message{
//     cursor: default;
// }